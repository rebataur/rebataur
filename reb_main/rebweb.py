...
def submit_config(path):
    if path == "pg":
        ...
        conn.commit
        for i in pgprops:
            sql = """insert into config(name,key,value) values('pg','%s','%s') """ % (
        ...
        for i in twitter_props:
            sql = """insert into service_reg(service_name,key_name,key, fk_id) values('%s','%s','%s',%s) """ %            (
                path, i, request.forms.get(i), 1)
            pg.execute_ddl(sql)
...
def query(path):
    ...
    if path == "save_keys":
        ...
        for i in range(0, 100):
            if request.forms.get(str(i)) is not None and request.forms.get(
                    str(i + 100)) is not None:
                arr = []
        ...
        for i in range(0, len(key_params)):
            sql = sql_tmpl % (fact_table_name, key_params[i][
                              0], "primary_key", key_params[i][1], "int")
            pg.execute_ddl(sql)
            sql = sql_tmpl % (fact_table_name, key_params[i][
                              0], "fact_key", key_params[i][2], "int")
            pg.execute_ddl(sql)

    elif path == "save_measures":
        pg.execute_ddl(
            "delete from facts_table where fact_table_name = '%s' and key_type ='measure_key';" %
            (fact_table_name))
        ...
        for i in range(0, len(key_params)):
            sql = sql_tmpl % (fact_table_name, key_params[i][
                              0], "measure_key", key_params[i][1], key_params[i][2])
            pg.execute_ddl(sql)
...
def create_star_schema(fact_table_name):
    ...
    pg = get_pg_conn()
    facts = pg.execute_dml(
        "select fact_table_name,table_name,key_type,key_val,dt_type from facts_table where fact_table_name = '%s' order by id asc;" %
        fact_table_name)
    ...
    for i in range(0, len(facts)):
        ...
        if column[2] == "primary_key" and i == 0:
            ...
            join_sql += " inner join %s %s on %s.%s = %s.%s" % (
                column[1], column[1][:2], col[:2], idx, column[1][:2], column[3])
            col = column[1]
...
def initialize():
    ...
    c = conn.cursor()
    c.execute(
        """ create table if not exists config(name varchar,key varchar,value varchar) """)
    conn.commit()
    c.close()
